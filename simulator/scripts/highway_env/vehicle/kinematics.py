...
class Vehicle(RoadObject):
    ...
    def step(self, dt: float) -> None:
        ...
        """
        self.clip_actions()
        rear_position = self.position - (self.LENGTH / 2) * np.array([np.cos(self.heading), np.sin(self.heading)])
        delta_f = self.action['steering']
        v = self.speed * np.array([np.cos(self.heading),
                                np.sin(self.heading)])
        rear_position += v * dt
        self.heading += self.speed * np.tan(delta_f) / (self.LENGTH) * dt
        self.speed += self.action['acceleration'] * dt
        self.position = rear_position + (self.LENGTH / 2) * np.array([np.cos(self.heading), np.sin(self.heading)])
